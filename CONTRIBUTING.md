...
- Naming conventions depend on which part of the project you wish to contribute to, but it's pretty standard to what's normal for each language.
    ...
    - For `/src/engine`, use `snake_case` for variables and procedures, `PascalCase` for types and enums, and `SCREAMING_SNAKE_CASE` for constants.
...
- Comments need to explain the why behind the code, not the how. When I say this, I mean that if, for example, you have a block of code that's verbose but it's for a good reason, you should explain *why* you did that in the comment, not *how* it works.
